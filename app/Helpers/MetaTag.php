...
class MetaTag
{
    ...
    public function __construct(Request $request, $defaultLocale, array $config = [])
    {
        ...
        // Set defaults
        $this->set('title', option('home_meta_title', $this->config['title']));
        $this->set('description', option('home_meta_desc', $this->config['description']));
        $this->set('url', $this->request->url());
        $this->set('image', asset('images/logo.png'));
        $this->set('type', 'website');
    ...
    public function twitterCard()
    {
        ...
        if (empty($html['image:src']) && $this->get('image')) {
            $html['image:src'] = $this->createMetaTag([
        ...
        if (empty($html['domain'])) {
            $html['domain'] = $this->createMetaTag([
                'name' => "twitter:domain",
    ...
    public function prependTitle($value)
    {
        ...
        return $this->metas['title'] = self::cut($value, $limit) . $title . ' | ' . $this->config['append_title'];
    }
     ...
     * @return string
     */
    private function createMetaTag(array $values)
    {
